...
class OrderBook extends EventEmitter {
  ...
  private bindPool = () => {
    if (this.pool) {
      ...
      this.pool.on('peer.nodeStateUpdate', (peer) => {
        // remove any trading pairs for which we no longer have swap client identifiers
        peer.activePairs.forEach((activePairId) => {
          const pairCurrencies = activePairId.split('/');
          let hasIdentifiers = true;
          pairCurrencies.forEach((currency) => {
            const currencyAttributes = this.getCurrencyAttributes(currency);
            if (!currencyAttributes || !peer.getIdentifier(currencyAttributes.swapClient, currency)) {
              hasIdentifiers = false;
            }
          });

          if (!hasIdentifiers) {
            // this peer's node state no longer has the swap identifiers for this trading pair
  ...
  private verifyPeerPairs = async (peer: Peer, pairIds: string[]) => {
    ...
    if (this.swaps) {
      ...
      const sanitySwapTimeout = setTimeoutPromise(15000, false);
